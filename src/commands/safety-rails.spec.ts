...
describe("pick --plan safety rail", () => {
  let stdoutSpy: ReturnType<typeof jest.spyOn>;
  let stderrSpy: ReturnType<typeof jest.spyOn>;
  ...
  beforeEach(() => {
    ...
    stderrSpy = jest.spyOn(process.stderr, "write").mockImplementation(() => true);
    mockGraphql.mockReset();
    mockLoadOptionsState.mockReset();
  ...
  it("throws VALIDATION when --plan does not match cached options tripPlanId", async () => {
    mockLoadOptionsState.mockReturnValue({ ...MOCK_OPTIONS_STATE, tripPlanId: "plan-123" });

    let err: unknown;
    try {
      await runPick(["1", "--plan", "plan-WRONG"]);
    } catch (e) {
      err = e;
    }
  ...
  it("mismatch error includes re-run hint", async () => {
    mockLoadOptionsState.mockReturnValue({ ...MOCK_OPTIONS_STATE, tripPlanId: "plan-ABC" });

    let err: unknown;
    try {
      await runPick(["1", "--plan", "plan-XYZ"]);
    } catch (e) {
      err = e;
    }

    expect((err as CliError).message).toContain("Re-run voyagier options plan-XYZ");
  });
  ...
  it("proceeds without error when --plan matches cached tripPlanId", async () => {
    ...
    mockGraphql.mockResolvedValue({
      setTripPlanSubSelectionOption: {
        ...
        selectedOption: { id: "opt-1", name: "Economy", price: 500 },
      },
    });

    await expect(runPick(["1", "--plan", "plan-123"])).resolves.toBeUndefined();
  });
});
