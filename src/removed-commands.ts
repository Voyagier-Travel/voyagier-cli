...
export const REMOVED_COMMANDS: readonly RemovedCommand[] = [
  ...
  { command: "plan-trip", tools: ["create_plan"], note: "Pass travellers as a JSON array with --travellers." },
  { command: "plan-status", tools: ["get_plan_status"] },
  { command: "plans create", tools: ["create_plan"] },
  { command: "plans list", tools: ["list_plans"] },
  ...
  { command: "plans collaborators", tools: ["list_collaborators"] },
  { command: "plans unshare", tools: ["remove_collaborator"] },
  { command: "plans shared", tools: ["list_plans"], note: "Use --relationship shared." },
  { command: "plans comments", tools: [] },
  { command: "plans vote", tools: [] },
  { command: "plans bookable", tools: ["get_plan_quote"] },
  { command: "plans goals", tools: ["get_plan_status"] },
  { command: "plans goal", tools: ["get_plan_status"] },
  { command: "plans goal-add", tools: ["add_goal"] },
  ...
  // Travellers
  { command: "travellers add", tools: ["add_travellers"], note: "Takes a JSON array of travellers." },
  { command: "travellers list", tools: ["list_travellers"] },
  { command: "travellers remove", tools: ["delete_traveller"] },
  { command: "travellers update", tools: ["update_traveller"] },
  // Search → select
  ...
  { command: "select", tools: ["select_option"], note: "Row-addressed picks: --participant_choice_id (rows from get_plan_choices)." },
  ...
  { command: "choices-view", tools: ["get_plan_choices"] },
  {
    ...
    note: "set_room_count sets how many rooms the hotel goal shops and books, on the goal's RoomArrangement (\"Number of Rooms\") selection (id from get_plan_choices or get_plan_status); each room slot row is then decided with select_option (row ids from get_plan_choices). set_room_rates only edits the rate list of an imported or hand-added room.",
  },
  { command: "traveller-choices list", tools: ["get_plan_choices"] },
  // Close
  { command: "cart", tools: ["get_plan_quote"] },
  { command: "quote", tools: ["get_plan_quote"] },
  { command: "send", tools: ["share_plan"], note: "share_plan returns the client link; you deliver it." },
  { command: "book", tools: ["book_plan"], note: "The price gate is --expect_total_cents (integer cents) plus --item_ids from get_plan_quote." },
  { command: "bookings list", tools: ["list_bookings"] },
  { command: "bookings get", tools: ["get_booking"] },
  // Account / clients
  { command: "whoami", tools: ["whoami"] },
  { command: "clients list", tools: ["list_clients"] },
