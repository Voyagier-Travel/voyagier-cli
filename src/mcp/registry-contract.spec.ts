...
const REMOTE_ONLY_TOOLS: Record<string, string> = {
  ...
  // 4.0 step C (VOY-2193): the CLI's own commands are now generated from the
  // remote tools/list, so every remote tool IS reachable as `voyagier <tool>`.
  // The stdio server's hand-written table (src/mcp/tools.ts) still lags; step D
  // (VOY-2194) replaces it with a proxy and collapses this spec to one check.
  set_date_range: "Reachable as `voyagier set_date_range`; the stdio server table is replaced by a proxy in step D.",
  set_destination: "See set_date_range.",
  promote_search: "See set_date_range.",
  search_status: "See set_date_range.",
  curate_options: "See set_date_range.",
  goal_delete: "See set_date_range.",
  share_plan: "See set_date_range.",
};
...
const REMOTE_PARAM_BASELINE: Record<string, string[]> = {
  ...
  choose_room_slot: ["create_new_choice", "for_all", "group_id", "option_id", "participant_choice_id", "replace_existing", "selection_id", "traveller_ids"],
  client_create: ["client_type", "email", "name"],
  clients_list: ["query"],
  curate_options: ["choose_listing_id", "listing_ids", "reset", "selection_id"],
  get_selection_options: ["selection_id"],
  goal_add: ["attach_to_journey_selection_id", "departure_date", "include_all_travellers", "name", "plan_id", "return_date", "round_trip", "scope", "sort_order", "type"],
  goal_delete: ["goal_id"],
  itinerary: ["plan_id"],
  plan_status: ["plan_id"],
  plan_trip: ["client_id", "days", "destination", "end_date", "start_date", "template", "title", "travel_destination_id", "travellers"],
  plans_list: ["limit", "page", "relationship"],
  promote_search: ["goal_id", "goal_name", "listing_id", "listing_ids", "new_goal", "plan_id", "search_id", "traveller_ids"],
  quote: ["plan_id"],
  refresh_options: ["force", "selection_id"],
  search_activities: ["category", "client_id", "currency", "date", "destination", "plan_id", "query"],
  search_destinations: ["query"],
  search_flights: ["adults", "children_ages", "client_id", "date", "from", "max_stops", "plan_id", "return", "to"],
  search_hotels: ["adults", "checkin", "checkout", "children_ages", "client_id", "currency", "hotel_name", "location", "plan_id", "rooms"],
  search_status: ["search_id"],
  select_option: ["option_id", "selection_id"],
  set_airport: ["apply_to_counterpart", "groups", "selection_id"],
  set_date_range: ["days", "end", "plan_id", "start"],
  set_destination: ["destination_id", "name", "plan_id"],
  share_plan: ["plan_id"],
  travellers_add: ["plan_id", "travellers"],
  travellers_list: ["plan_id"],
  travellers_update: ["date_of_birth", "email", "first_name", "gender", "last_name", "passport", "traveller_id"],
};
