...
export const INSTRUCTIONS = [
  ...
  "",
  "book REQUIRES a price gate: pass expect_total_cents (integer cents, preferred — the same form the remote Voyagier MCP server takes) or expect_total (dollars); max_total alone is also accepted as a cap-only gate. book creates a real Stripe checkout only if the chargeable subtotal equals that total exactly; otherwise it fails closed with PRICE_CHANGED and no checkout is created. Get the current subtotal from book_dry_run first, then book with that exact total. Never retry a successful book.",
  "",
