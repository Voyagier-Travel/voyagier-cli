...
### Changed
- **Options digest rows carry a `bookability` state instead of `isBookable`.** The server replaced the row boolean on `optionsSummary.topOptions` (`search_flights`, `search_hotels`, `search_activities`, `get_search_status`, `promote_search`, `get_options`) with `bookability: exploration | decision | bookable | unavailable` and added a one-sentence `optionsSummary.nextStep`. The human renderers tag each row with its state (`exploration`, `decision`, `unavailable`; a `bookable` leaf is untagged) and print the next step as `next: …`; the old `not bookable` tag — which read as a verdict on rows that are never the bookable unit — is gone. `--json` output is the server payload as sent. AGENT.md gains a "Reading search results" note. Checked-in registry snapshot refreshed to the matching server registry (`search_flights`, `search_hotels`, `get_options` descriptions; optional-parameter descriptions and other descriptor drift caught up from the server's main).
