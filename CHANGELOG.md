...
### Changed
- **Checked-in registry snapshot** (`src/mcp/fixtures/remote-tools.json`) refreshed from the server's registry export: the `search_hotels` description adds that an ambiguous city name needs its country (`Montmartre, Paris, France`). Every other descriptor is unchanged. AGENT.md's hotel-search line says the same. Requires the matching server change (VOY-2350).
