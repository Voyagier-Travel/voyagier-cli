...
### Changed
- **Checked-in registry snapshot** (`src/mcp/fixtures/remote-tools.json`) refreshed from the server's registry export: the `search_hotels` description says the 3-mile anchor for a `District, City` query holds when the city is a known destination (read `distanceMi` to confirm) and that an ambiguous city name needs its country (`Montmartre, Paris, France`). Every other descriptor is unchanged. AGENT.md's hotel-search line says the same. Requires the matching server change (VOY-2350).
- **Options digest rows carry `bookable` + `bookableReason` only where they book as themselves.** On `optionsSummary.topOptions` (`search_flights`, `search_hotels`, `search_activities`, `get_search_status`, `promote_search`, `get_options`) the row boolean `isBookable` is gone. A row that books as itself — a Fare & Cabin fare, a hotel room rate, an activity option, or an imported or custom item — carries `bookable` + `bookableReason`, the pair `get_plan_quote` lines already used (`bookableReason` names the cause: already reserved / booked / paid on this plan, or not offered for booking right now). Every other row, and every search-result row, carries no bookable key at all. The human renderers print a leaf's verdict with the server's reason and nothing on the other rows; the old blanket `not bookable` tag on every row is gone. `get_plan_status` (`summary.bookableNow`, `cart.bookableCount`), `get_plan_choices` (`isBookable`), `get_plan_quote` and `update_option --is_bookable` are unchanged. `--json` output is the server payload as sent. AGENT.md gains a "Reading search results" note; the path from a result to a booking is in the `search_flights`, `search_hotels`, `search_activities` and `get_options` descriptions. Checked-in registry snapshot (`src/mcp/fixtures/remote-tools.json`) refreshed to the matching server registry; the same refresh carries the server's earlier `delete_plan` → `archive_plan` rename (archiving hides a plan from its owner, client and collaborators instead of deleting it — `voyagier archive_plan --plan_id <PLAN_ID>`; the 3.x migration map and AGENT.md point at it) and the optional-parameter descriptions the server added since the previous snapshot. Generated command flags that move with this refresh: `add_guide_event` / `update_guide_event` gain `--media_id` (event card image, a media object id from `list_plan_media_objects`; null restores the default on update); `add_guide_comment` / `update_guide_comment` gain `--end_day_date` (multi-day day notes) and `update_guide_comment` gains `--day_date` with `--text` now optional; `delete_client` is present (81 tools).
...
### Added
- **Nullable flags accept `null`.** An input the schema declares nullable (`type: ["string", "null"]` or `anyOf` with a null member — `update_plan --description` / `--cover_media_id`, `update_guide_block --place_id` / `--start_place_id` / `--end_place_id`, `update_guide_event --description` / `--start_local` / `--local_time` / `--duration_minutes`) keeps its base type and additionally takes the literal `null`, sent as JSON null so the server's "pass null to clear" works from the CLI. `--help` marks these flags `(pass null to clear)`; `anyOf` unions with one non-null member are typed like that member instead of falling back to a JSON literal. A nullable array of strings or numbers stays a repeatable flag and takes `null` as its only value (`--x null` clears the list; `null` next to another value is an error). On every other flag `null` gets no special treatment: a string flag sends the text `null`; a number, enum or object flag rejects it like any other invalid input.
...
- **Option ids are validated before the request, and an empty pick response is reported as an error.** `select --option-id`, `choose-room-slot --option-id`, and the `select_option` / `choose_room_slot` MCP schemas now require the full 36-character UUID printed by `search` / `selection-options`. A shortened id satisfies the mutation's `String` argument but matches no option, so the pick came back empty with nothing selected; it is now rejected locally with `VALIDATION`. An empty pick or participant-choice payload is reported as `API_ERROR` — with the command to re-fetch the selection's options — instead of a success-shaped result.
