...
### Changed
- **Verb-first tool names.** The server's tools are now `create_plan`, `list_plans`, `get_plan_status`, `get_plan_itinerary`, `get_plan_quote`, `book_plan`, `get_search_status`, `get_options`, `get_plan_choices`, `add_goal` / `update_goal` / `delete_goal`, `list_clients` / `get_client` / `create_client` / `update_client`, `add_travellers` / `list_travellers` / `update_traveller` / `delete_traveller`, `list_bookings` / `get_booking`, `set_plan_dates`, `list_collaborators` / `remove_collaborator`, `update_plan` / `delete_plan`. `choose_room_slot` is gone: `set_room_count` sets how many rooms the hotel selection needs; each room slot row is then decided with `select_option --participant_choice_id` (rows from `get_plan_choices`). `set_room_rates` only edits the rate list of an imported or hand-added room. The 3.x removal messages, the migration table below (regenerated in the 4.0.0 section), AGENT.md, README and the `--help` quick start use the new names.
- **Human renderers** follow the rename: `get_plan_status`, `get_search_status`, `get_options`, `get_plan_itinerary` and `get_plan_quote` render the compact view that `plan_status`, `search_status`, `get_selection_options`, `itinerary` and `quote` had. `refresh_options` now returns `true` and prints JSON like every other tool; it no longer shares the `get_options` renderer. The copy-pasteable acceptance line reads `voyagier book_plan …`. `get_options` also lists the selection's decision rows (`participantChoices`: `participant_choice_id`, travellers, decided/undecided, current option) with a `select_option --participant_choice_id …` line for the first undecided row, and ends with `more options: voyagier get_options … --cursor <nextCursor>` when the server reports another page, repeating the `--query` (shell-quoted) and `--limit` the page was read with, as the tool's contract asks. A queried digest counts only matching options as "more"; when the page already shows every match it reads `(M matching of C total)`.
- **Results are the bare payload.** The server now returns each tool's payload object directly instead of `{ "<operation>": payload }`; `--json` prints exactly that. The renderers unwrap the old envelope only when a single root key looks like a GraphQL operation name and is not a field of the payload, so a genuine one-field result is never unwrapped.
...
- **Upgrading invalidates the cached tool table.** `tools-cache.json` records the CLI version that wrote it; an entry from another version (or from 4.0, which recorded none) is not used, so the first command after an upgrade lists the tools again instead of showing the previous release's names for up to 24 hours. `doctor` and `--verbose` say when a previous entry was ignored for that reason. `agent-docs` applies the same rule to its stale-cache fallback: without credentials, or when the fetch fails, it never prints instructions cached by another CLI version and its note says which version wrote them.
...
| `voyagier choose-room-slot` | `voyagier set_room_count`, `voyagier select_option` | set_room_count sets how many rooms the hotel selection needs; each room slot row is then decided with select_option --participant_choice_id (rows from get_plan_choices). set_room_rates only edits the rate list of an imported or hand-added room. |
