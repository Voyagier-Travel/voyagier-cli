...
- **One bookable vocabulary across the server's payloads.** On `optionsSummary.topOptions` (`search_flights`, `search_hotels`, `search_activities`, `get_search_status`, `promote_search`, `get_options`) the row boolean `isBookable` is gone: every row carries `stage: exploration | on_plan`, and only leaf rows (Fare & Cabin fare, room rate, activity option) carry `bookable` + `bookableReason` — the pair `get_plan_quote` lines already used. `optionsSummary.nextStep` is a one-sentence server next step. `get_plan_status` reports `summary.bookable` + `summary.bookableReason` (was `bookableNow`) and `cart.bookableItemCount` (was `bookableCount`); `get_plan_choices` rows read `bookable`; `update_option` takes `--bookable` (was `--is_bookable`). The human renderers tag exploration rows, print a leaf's verdict with the server's reason, print the next step as `next: …`, and read the renamed plan-status fields; the old blanket `not bookable` tag on every row is gone. `--json` output is the server payload as sent. AGENT.md gains a "Reading search results" note. Checked-in registry snapshot (`src/mcp/fixtures/remote-tools.json`) refreshed to the matching server registry; the same refresh carries the server's earlier `delete_plan` → `archive_plan` rename (archiving hides a plan from its owner, client and collaborators instead of deleting it — `voyagier archive_plan --plan_id <PLAN_ID>`; the 3.x migration map and AGENT.md point at it) and the optional-parameter descriptions the server added since the previous snapshot. Generated command flags that move with this refresh: `update_option --bookable` (was `--is_bookable`); `add_guide_event` / `update_guide_event` gain `--media_id` (event card image, a media object id from `list_plan_media_objects`; null restores the default on update); `add_guide_comment` / `update_guide_comment` gain `--end_day_date` (multi-day day notes) and `update_guide_comment` gains `--day_date` with `--text` now optional; `delete_client` is present (81 tools).
