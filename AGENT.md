...
- **Reading search results.** Every `optionsSummary.topOptions` row carries `stage`: `exploration` (a `search_flights` / `search_hotels` / `search_activities` row, on no plan) or `on_plan` (a plan selection row). `stage` says nothing about booking. Only rows that book as themselves — a Fare & Cabin fare, a hotel room rate, an activity option, or an imported or custom item — carry `bookable` + `bookableReason`, the same pair `get_plan_quote` lines and `get_plan_status.summary` carry (`cart.bookableItemCount` counts the cart's bookable lines). A flight journey, hotel, room or activity row carries no bookable flag at all: its pick creates the child choice that holds the leaf (a journey → the Fare & Cabin row; a room → the rate row; an activity → its activity-option row), so an exploration or decision list is a list to choose from, never a verdict. `optionsSummary.nextStep` is the server's one-sentence next step for the list in hand (it says to re-run the search when the record has expired); the human view prints it as `next:`, tags exploration rows, and prints a leaf's verdict with the server's reason. To book an exploration row: `promote_search` with its `listing_id` (flights: `goal_id` = the plan's outbound flight goal; the listing decides both legs and the Fare & Cabin row auto-decides on the first leg pick — read it with `get_options`; hotels: then `select_option` a room from `get_plan_choices` — its rate row is decided and carted for you, change it on the rate row; activities: then `select_option` its activity-option row from `get_plan_choices`) → `get_plan_quote` → `book_plan`. A search record past its `expiresAt` is re-run, not promoted; `nextStep` says so with the expiry. `update_option --bookable` sets the leaf flag on a manual option.
...
**Hotel search.** For a neighbourhood, write `--location "District, City"` (`"Shinjuku, Tokyo"`) — the search anchors there at 3 mi when the city is a known destination (read `distanceMi` to confirm) — and pass `--country JP` when you know it; when the city name is ambiguous, add the country (`"Montmartre, Paris, France"`). A hotel row carries `distanceMi` (miles from the searched point) when the server could place the property; treat it as optional and never assume it on every row. `--radius_miles` overrides the default radius.
