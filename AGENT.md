...
- **Reading search results.** Every `optionsSummary.topOptions` row carries `bookability`: `exploration` (a `search_flights` / `search_hotels` / `search_activities` row, on no plan), `decision` (a plan row whose pick creates the child choice that holds the bookable leaf), `bookable` (a leaf that carts and books as-is) or `unavailable` (the supplier says so). A flight journey or hotel row is never the bookable unit, on a plan or off it: the Fare & Cabin fare and the room rate are, so do not report an `exploration` or `decision` list as not bookable. `optionsSummary.nextStep` is the server's one-sentence next step for the list in hand; the human view prints it as `next:` and tags each row with its state. To book an exploration row: `promote_search` (with its `listing_id`, and `goal_id` = the plan's outbound flight goal for flights) → `select_option` → decide the Fare & Cabin or rate choice it creates → `get_plan_quote` → `book_plan`. Cart and quote items (`get_plan_quote`) keep their own `bookable` flag; that one is a real purchasable-item verdict.
