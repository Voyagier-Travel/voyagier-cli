...
- **Reading search results.** Only rows that book as themselves — a Fare & Cabin fare, a hotel room rate, an activity option, or an imported or custom item — carry `bookable` + `bookableReason` on `optionsSummary.topOptions`, the same pair `get_plan_quote` lines carry. Every other row carries no bookable key at all: a `search_flights` / `search_hotels` / `search_activities` result row, and a flight journey, hotel, room or activity row on a plan whose pick creates the child choice that holds the leaf (a journey → the Fare & Cabin row; a room → the rate row; an activity → its activity-option row). A list without bookable keys is a list to choose from, never a verdict; the human view prints a leaf's verdict with the server's reason and nothing on the other rows. The path from a result to a booking is in the tool descriptions (`search_flights`, `search_hotels`, `search_activities`, `get_options` — `voyagier <tool> --help`): `promote_search` with the `listing_id` → the pick that creates the leaf → `get_plan_quote` → `book_plan`; a search record past its `expiresAt` is re-run, not promoted. `get_plan_status` keeps `summary.bookableNow` and `cart.bookableCount`; `get_plan_choices` rows keep `isBookable`; `update_option --is_bookable` sets the flag on a manual option.
