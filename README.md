...
> **Tip:** prefer `voyagier login` (interactive prompt) over `auth set-token <token>` — it keeps your token out of shell history. For scripts, use the `VOYAGIER_TOKEN` env var or `auth set-token -` (stdin).
