...
- **Output.** `--json` prints the tool's result as JSON: the payload object itself, with no wrapper key. Without it, `get_plan_status`, the `search_*` tools, `get_options`, `get_plan_itinerary` and `get_plan_quote` render a compact human view; other tools pretty-print the JSON.
...
- **Tool cache.** The server's tool list is cached in `~/.voyagier/tools-cache.json` for 24 hours. `voyagier doctor` refreshes it, and so does running a tool name the cache does not know yet; a cache written by another CLI version is not used, so an upgrade lists the tools again on first use. `doctor` and `--verbose` report a stable hash of the tool surface (names + input schemas) so scripts can detect a contract change.
